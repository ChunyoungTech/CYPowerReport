<%@ Page Title="" Language="C#" MasterPageFile="~/_master/Grid.master" AutoEventWireup="true" CodeBehind="TagAlarmStatus.aspx.cs" Inherits="WebApp._alarm.TagAlarmStatus" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    <script type="text/javascript">
        var extOpt = [{ reCtl: "#<%=lbRefresh.ClientID%>", reHid: "#hidRefresh", Width: .8, Sub: "41" }];
        InitExt(extOpt);

        function stopAlarm() {
            $.ajax({
                type: "GET",
                url: "<%=cyc.Comm.SysQuery.GetAppSettingValue("stopAlarmUpdater")%>",
        ...
        function startAlarm() {
            $.ajax({
                type: "GET",
                url: "<%=cyc.Comm.SysQuery.GetAppSettingValue("startAlarmUpdater")%>",
                success: function (data) {
                    alert(data.message);
                }
            });
