    ...
    public class GetDepUser : cyc.Page.BaseHandler
    {
        ...
        protected override void DoHandler(HttpContext context)
        {
            ...
            else if (string.IsNullOrEmpty(context.Request.QueryString["Emp"]))//查詢單位內部所有人員
            {
                ...
                {
                    var uList = oDB.QueryList<cyc.SysUser>("select ID,Name from SysUser where DeptID=@Dep", new { Dep = Convert.ToInt32(context.Request.QueryString["Dep"]) });
                    context.Response.Write(this.SerializeObject(uList));
                }
            }
            else//依員工班號或姓名查詢
            {
                using (cyc.DB.SqlDapperConn oDB = new cyc.DB.SqlDapperConn(oResult))
                {
                    var uList = oDB.QueryOne<cyc.SysUser>("select ID,Name from SysUser where Name=@Name", new { Name = context.Request.QueryString["Emp"].ToString() });
                    context.Response.Write(this.SerializeObject(uList));
                }
            }
        }
