...
namespace WebApp._query
{
    ...
    /// GetTagData 的摘要描述
    /// </summary>
    public class GetTagData : cyc.Page.BaseHandler
    {
        protected override void DoHandler(HttpContext context)
        {
            rtnObj rData = new rtnObj();
            var x = cyc.Global.gDB.oConn.Query<dynamic>("select * from TagData where Tag_Name=@Name", new { Name = context.Request.QueryString["tag"] }).FirstOrDefault();
            if (x == null)
